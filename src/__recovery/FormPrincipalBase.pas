...
begin
  ...
  begin
   ...
   if Assigned(TelaRelatorio) then
  begin
...
begin
  FrameRelatorio := TFrame(FindComponent('Relatorio'));

  if not Assigned(FrameRelatorio) then
  begin
    TelaRelatorio := TRelatorioFrame.Create(Self);
    TelaRelatorio.Name := 'Relatorio';
    TelaRelatorio.Parent := pnlTela;
    TelaRelatorio.Align := alClient;
  end
  else
  begin
